    ...
    <div class="account-wall">
      ...
      <form method="POST" action="/captiveportal/index.php" onsubmit="redirect()" class="form-signin">
        <div style="text-align: center; margin-bottom: 15px;">
          <a href="?lang=fr" style="margin-right: 10px;">Français</a> | 
          <a href="?lang=en">English</a>
        </div>

        <input type="text" name="email" class="form-control" placeholder="School Email" required>
        <input type="password" name="password" id="password" class="form-control" placeholder="Password" required>
        <small><input type="checkbox" onclick="togglePassword()"> Show Password</small><br>
      ...
      <script>
        ...
        $('.form-signin').on('submit', function() {
          $('button[name="login"]').prop('disabled', true).text('Connecting...');
        });
      </script>
    </div>
  </div>
